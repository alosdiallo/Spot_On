...
sub check_options {
    my $opts = shift;
    my ($inputPNG,$nClusters,$smoothRadius,$smoothMode,$colonyMinSize,$colonyMaxSize,$colonyNeighbors,$debugMode);
	...
	return($inputPNG,$nClusters,$smoothRadius,$smoothMode,$colonyMinSize,$colonyMaxSize,$colonyNeighbors,$debugMode);
}
...
my $results = GetOptions( \%options,'inputPNG|i=s','nClusters|k=s','smoothRadius|r=s','smoothMode|m=s','colonyMinSize|min=s','colonyMaxSize|max=s','colonyNeighbors|nhood=s','debugMode|d=s');
my ($inputPNG,$nClusters,$smoothRadius,$smoothMode,$colonyMinSize,$colonyMaxSize,$colonyNeighbors,$debugMode);
($inputPNG,$nClusters,$smoothRadius,$smoothMode,$colonyMinSize,$colonyMaxSize,$colonyNeighbors,$debugMode)=check_options( \%options );
...
my ($debugName,$testName);
$debugName = $inputPNG;
$debugName =~ s/.png$//;
$debugName .= "_DEBUG";
system("mkdir -p $debugName");
$testName=$debugName."/".$inputPNG;
...
matrix2TXT($matrix,$height,$width,$testName.".orig.txt") if($debugMode == 1);
...
matrix2TXT($smoothedMatrix,$height,$width,$testName.".smoothed.txt") if($debugMode == 1);
...
matrix2TXT($correctedMatrix,$height,$width,$testName.".corrected.txt") if($debugMode == 1);
...
matrix2PNG($kmeansMatrix,$height,$width,0,$testName.".kmeans.png") if($debugMode == 1);
#convert matrix to PNG image
matrix2TXT($kmeansMatrix,$height,$width,$testName.".kmeans.txt") if($debugMode == 1);
...
my $backgroundClusterNumber = (sort { $objects->{$b} <=> $objects->{$a} } keys %$objects)[0];
print "\tusing object # $backgroundClusterNumber as background...\n";
delete($objects->{$backgroundClusterNumber});
#convert matrix to TXT file
matrix2PNG($objectMatrix,$height,$width,$backgroundClusterNumber,$testName.".object.png") if($debugMode == 1);
#convert matrix to PNG image
matrix2TXT($objectMatrix,$height,$width,$testName.".object.txt") if($debugMode == 1);
...
#convert matrix to TXT file
matrix2TXT($cleanMatrix1,$height,$width,$testName.".clean.step1.txt") if($debugMode == 1);
#convert matrix to PNG image
matrix2PNG($cleanMatrix1,$height,$width,$backgroundClusterNumber,$testName.".clean.step1.png") if($debugMode == 1);
...
#convert matrix to TXT file
matrix2TXT($cleanMatrix2,$height,$width,$testName.".clean.step2.txt") if($debugMode == 1);
#convert matrix to PNG image
matrix2PNG($cleanMatrix2,$height,$width,$backgroundClusterNumber,$testName.".clean.step2.png") if($debugMode == 1);
...
#convert matrix to TXT file
matrix2TXT($centerMatrix,$height,$width,$testName.".clean.centers.txt") if($debugMode == 1);
#convert matrix to PNG image
matrix2PNG($centerMatrix,$height,$width,$backgroundClusterNumber,$testName.".clean.centers.png") if($debugMode == 1);
...
#convert matrix to TXT file
matrix2TXT($centerLineMatrix,$height,$width,$testName.".clean.centers.lines.txt") if($debugMode == 1);
#convert matrix to PNG image
matrix2PNG($centerLineMatrix,$height,$width,$backgroundClusterNumber,$testName.".clean.centers.lines.png") if($debugMode == 1);
...
print "overlaying outer grid lines on input matrix...\n";
my $outerLineMatrix=[];
$outerLineMatrix=overlayLine($centerLineMatrix,$height,$width,$backgroundClusterNumber,$yOuterLine,$xOuterLine,-3);
#convert matrix to TXT file
matrix2TXT($centerLineMatrix,$height,$width,$testName.".clean.centers.lines.outer.txt") if($debugMode == 1);
#convert matrix to PNG image
matrix2PNG($centerLineMatrix,$height,$width,$backgroundClusterNumber,$testName.".clean.centers.lines.outer.png") if($debugMode == 1);
...
my $colonyData=[];
my $finalMatrix=[];
($colonyData,$finalMatrix)=processSubColony($matrix,32,48,$backgroundClusterNumber,$yColonyBoundary,$xColonyBoundary,$centerLineMatrix);
#convert matrix to TXT file
matrix2TXT($finalMatrix,$height,$width,$testName.".clean.centers.lines.outer.final.txt") if($debugMode == 1);
#convert matrix to PNG image
...
$dataName=$dirName."/".$inputPNG;

print "printing final colony size per grid box...\n";
matrix2TXT($colonyData->{'size'},32,48,$dataName.".size.txt");

print "printing all final 'colony' data...\n";
#print all values of pixels of a detectable colony
matrix2TXT($colonyData->{'blue'}->{'mean'}->{'colony'},32,48,$dataName.".blue.mean.colony.txt");
matrix2TXT($colonyData->{'blue'}->{'stdev'}->{'colony'},32,48,$dataName.".blue.stdev.colony.txt");
matrix2TXT($colonyData->{'blue'}->{'median'}->{'colony'},32,48,$dataName.".blue.median.colony.txt");
matrix2TXT($colonyData->{'red'}->{'mean'}->{'colony'},32,48,$dataName.".red.mean.colony.txt");
matrix2TXT($colonyData->{'red'}->{'stdev'}->{'colony'},32,48,$dataName.".red.stdev.colony.txt");
matrix2TXT($colonyData->{'red'}->{'median'}->{'colony'},32,48,$dataName.".red.median.colony.txt");
matrix2TXT($colonyData->{'green'}->{'mean'}->{'colony'},32,48,$dataName.".green.mean.colony.txt");
matrix2TXT($colonyData->{'green'}->{'stdev'}->{'colony'},32,48,$dataName.".green.stdev.colony.txt");
matrix2TXT($colonyData->{'green'}->{'median'}->{'colony'},32,48,$dataName.".green.median.colony.txt");

print "printing all final 'box' data...\n";
#print all values of the entire 'box'
matrix2TXT($colonyData->{'blue'}->{'mean'}->{'all'},32,48,$dataName.".blue.mean.all.txt");
matrix2TXT($colonyData->{'blue'}->{'stdev'}->{'all'},32,48,$dataName.".blue.stdev.all.txt");
matrix2TXT($colonyData->{'blue'}->{'median'}->{'all'},32,48,$dataName.".blue.median.all.txt");
matrix2TXT($colonyData->{'red'}->{'mean'}->{'all'},32,48,$dataName.".red.mean.all.txt");
matrix2TXT($colonyData->{'red'}->{'stdev'}->{'all'},32,48,$dataName.".red.stdev.all.txt");
matrix2TXT($colonyData->{'red'}->{'median'}->{'all'},32,48,$dataName.".red.median.all.txt");
matrix2TXT($colonyData->{'green'}->{'mean'}->{'all'},32,48,$dataName.".green.mean.all.txt");
matrix2TXT($colonyData->{'green'}->{'stdev'}->{'all'},32,48,$dataName.".green.stdev.all.txt");
matrix2TXT($colonyData->{'green'}->{'median'}->{'all'},32,48,$dataName.".green.median.all.txt");
